...
async function runHttp(): Promise<void> {
  const app = express();
  const host = process.env.LIVING_BODY_MCP_HOST ?? "127.0.0.1";
  const port = Number(process.env.LIVING_BODY_MCP_PORT ?? 3030);
  const allowedOrigin = process.env.LIVING_BODY_MCP_ALLOWED_ORIGIN ?? `http://${host}:${port}`;
