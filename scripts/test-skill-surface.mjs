...
assert.match(skill, /call living_body_connection_status/);
...
const result = await run(["call", "living_body_connection_status", "--json", "{}"]);
...
console.log(JSON.stringify({ ok: true, suite: "skill-surface", tool: "living_body_connection_status", files: pkg.files }, null, 2));
